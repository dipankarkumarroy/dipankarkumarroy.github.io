    ...
    return this.each( function() {
      ...
      var gradPicker = $(idref + '> .jGraduate_gradPick');
        ...
        '</div>' +
            ...
            '</div>' +
            '<label><input type="text" id="' + id + '_jGraduate_AngleInput" size="3" value="0"/>Ã‚Âº&nbsp;</label>' + 
                '</div>' +
...
MD.Keyboard = function(){
  ...
  for (const key in keys) {
    ...
    chords.forEach(key => {
      ...
      if (key === " ") key = "SPACEBAR";
      if (key === "shift") key = "â‡§";
      if (key === "cmd") key = svgedit.browser.isMac ? "âŒ˜" : "Ctrl";
